...
from model_lib.efficientnet_pytorch import EfficientNet as EffNet
from model_lib.efficientnet_pytorch.utils import MemoryEfficientSwish, Swish
from model_lib.efficientnet_pytorch.utils_extra import Conv2dStaticSamePadding, MaxPool2dStaticSamePadding
...
class Regressor(nn.Module):
    ...
    def __init__(self, in_channels, num_layers, pyramid_levels=5, onnx_export=False):
...
class NewEfficientJCOBackbone(nn.Module):
    def __init__(self, num_classes=4, compound_coef=0, load_weights=False, **kwargs):
        super(NewEfficientJCOBackbone, self).__init__()
        ...
        self.conv_list = nn.ModuleList([SeparableConvBlock(64, 64, norm=False, activation=False) for j in range(5)])
        self.bn_list = nn.ModuleList([nn.BatchNorm2d(64, momentum=0.01, eps=1e-3) for j in range(5)])
    ...
    def forward(self, inputs):
        ...
        feats = []
        for feat, bn, conv in zip(features, self.bn_list, self.conv_list):
            feat = conv(feat)
            feat = bn(feat)
            feat = self.swish(feat)
        ...
        feats = torch.cat(feats, dim=1).flatten(start_dim=1)
        c_out = self.classifier_(feats)
        r_out = self.regressioner_(feats)[:, 0]
